...
%token FOR WHILE DO IF ELSE TRU FALS <num>INTEGER_N <numf>FLOAT_N  <str>F <str>D PER SYMB <str>TURN <str>FUNC <str>VOID
%token AF AO FP EQ OP DIV MUL SUB ADD MOD <str>PUI <str>AFF INF SUP DIFF EG SE IE INCR DEC VRG PVG DP
  
%%  
PROGRAMME : FONCTION DECLARATION BEGINN INSTRUCTION END { printf ("\n programme syntaxiquement juste");YYACCEPT;}
;

FONCTION: TYPEF FUNC IDF OP DECLARATION FP AO DECLARATION INSTRUCTION  RETURN T PVG AF {insererTYPE($3,sauv);}
        |
;


TYPEF :INT {strcpy(sauv,$1);} 
|FLOAT {strcpy(sauv,$1);} 
|VOID{strcpy(sauv,$1);} 
|BOOL{strcpy(sauv,$1);} 
|
...
INSTRUCTION:INST_FOR INSTRUCTION  
           ...
           |APPEL_INST INSTRUCTION
           |
;

APPEL_INST : IDF OP T FP PVG 
;

EVRG: VRG  IDF EVRG {if(DoubleDec($2)==0) printf("erreur semantique a la ligne %d et la colonne %d la variable %s nest pas declarer \n",nb_ligne,nb_col,$1);}
| 
;
...
|IDF AFF APPEL_INST{if(DoubleDec($1)==0){printf("erreur semantique la ligne %d et la colonne %d la variable %s nest pas declaree \n",nb_ligne,nb_col,$1);}}
; 
